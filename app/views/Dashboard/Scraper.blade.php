		...
		<div class="content row col-sm-11 settings">
				<div style="text-align: center;">
					...
					<div class="well2">
						...
						<p>Scraped in Past 24 hrs</p>
					</div>
					<div class="well2">
						<h2 class="total_scraped_count">0</h2>
						<p>Total Fully Scraped</p>
					</div>
					<br/>
					<a href="/dashboard/title-scraper?mode=real-time" class="btn <?php echo Input::get('mode') != 'static-log' ? 'btn-primary active' : '' ?>">Real-Time Log</a>
					<a href="/dashboard/title-scraper?mode=static-log" class="btn  <?php echo Input::get('mode') == 'static-log' ? 'btn-primary active' : '' ?>">Static Log</a>
				...
				<div class="pagination" style="padding:20px;text-align:center;display:none;">
					<button onclick="prevPage();" class="btn"><</button> &nbsp; Page #<span class="page-number">1</span> of <span class="page-count">0</span> &nbsp; <button onclick="nextPage();" class="btn">></button>
				</div>
			</div>
		</section>
	</body>
...
<script type="text/javascript">
	...
	function nextPage()
	{
		var current = parseInt($('span.page-number').html());

		$('span.page-number').html(current + 1);
		getLogPage();
	}
	...
	function getLogPage()
	{
		var page = parseInt($('span.page-number').html());

		$.post('/scraper/log', {page: page}, function(json) {
			$('table tr.log').remove();
	...
	function updateScraper()
	{
		...
		$.post('/scraper/log', { lastLogId: lastLogId }, function(json) {
		...
		<?php if(Input::get('mode') == 'real-time' || Input::get('mode') == null): ?>
			setTimeout(updateScraper, 1500);
		<?php else: ?>
			$('div.pagination').show();
		<?php endif; ?>
	}
