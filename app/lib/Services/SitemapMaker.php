...
class SitemapMaker {
	...
	private function makeIndex(array $index)
	{
	    ...
	    foreach ($index as $resource => $number)
	    {
	    	for ($i=1; $i <= $number; $i++)
		    { 
		        $sitemap->addSitemap(url("{$resource}-sitemap-$i"));
		    }
	    }

	    $sitemap->addSitemap(url("static-pages-sitemap"));
	...
	private function makeDynamicMaps($name, array $columns)
	{
	    ...
	    {    
	        ...
	        $resources = DB::table($name)->orderBy('created_at', 'desc')->skip($done)->take($this->queryLimit)->get($columns);

	        foreach ($resources as $resource)
	        {
	            $sitemap->add(
	            	Helpers::url($name == 'actors' ? $resource->name : $resource->title, $resource->id, $name == 'titles' ? $resource->type : $name),
	            	$resource->updated_at, 1, 'weekly'
	            );
	            
	        }

	        $sitemap->store('xml', "{$name}-sitemap-$fileNum");
