...
class Writer
{
	...
	public function __construct($provider=null, $flags=array())
	{
		...
		$this->images = App::make('Lib\Services\Images\S3ImageSaver');
	...
	private function saveImdbImages(array $images)
	{
		...
		{
			$imgs[] = array(
				'local' => "imdb/stills/{$id}$k.jpg",
				'title_id' => $this->titleId,
				'web'		=>	$this->images->linkTo("imdb/stills/{$id}$k.jpg")
			);
	...
	private function addPoster()
	{
		...
		if ($this->genInfo['poster'])
		{
			...
			$this->genInfo['poster'] = $this->images->linkTo("imdb/posters/$id.jpg");
		}
		else
		{
	...
	private function addJumboBackground()
	{
		if ($this->provider->name == 'imdb' || $this->options->saveTmdbImages())
		{
			...
			if ($img)
			{
				...
				$this->genInfo['background'] = $this->images->linkTo("imdb/bgs/{$id}.jpg");
			}		
		}
		else
		{
	...
	private function saveImdbCastImages(array $actors)
	{
		...
		foreach ($actors as $k => $v)
		{
			if ($v['image'])
			{
				...
				$actors[$k]['image'] = $this->images->linkTo("imdb/cast/{$id}.jpg");				
        ...
        if ($this->options->saveTmdbImages())
        {
	        ...
	        foreach ($titles as $k => $v)
			{
				if ($v['poster'])
				{
					...
					$titles[$k]['poster'] = $this->images->linkTo("imdb/posters/{$id}.jpg");				
					$urls[$id] = $v['poster'];
				}		
			}
    ...
    private function prepareImdbInsert(array $titles, $tempId)
    {
        ...
        {             
            ...
            if ($v['poster'])
            {
                ...
                {
                     $titles[$k]['poster'] = $this->images->linkTo("imdb/posters/{$v['imdb_id']}" . '.jpg');
                }        	
            }                  
        }
