...
use Lib\Services\Images\S3ImageSaver as Imgs;
...
class TitleRepository extends Repository
{
     ...
     * @return Collection
     */
    public function newAndUpcoming($limit = 5)
    {   
    ...
    {   
        return $this->model->with('Link')
                    ->whereNotNull('poster')
                    ->where('release_date', '>=', Carbon::now()->addYears(-5))
                    ->orderBy('imdb_rating', 'desc')
                    ->remember(8640, 'titles.topRated')
                    ->limit($limit)
                    ->get();
    }
    ...
    protected function appendParams(array $params, $query)
    {
        ...
        {
            $query = $query->where('imdb_rating', '>=', (int) $params['minRating']);
        }

        if ((isset($params['maxRating']) && $params['maxRating']) && (int) $params['maxRating'] !== 10)
        {
            $query = $query->where('imdb_rating', '>=', (int) $params['maxRating']);
        }
